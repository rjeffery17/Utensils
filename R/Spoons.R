...
FACSPlotter <- function(FACS.data, x.axis, y.axis, colour.by, colour.by.title = "Condition", x.title = "Day", y.title = "FOXP3 T Cells (%)", legend.position = "right", facet = NULL, facet_scales = "free_x", xblank = FALSE, y.log = FALSE, colour.palette = c("#494949ff", "#7900f5ff"), facet_rows = 1, x.axis.angle = NULL){
  Plot <- ggplot(FACS.data, aes_string(x = x.axis, y = y.axis, color = colour.by)) +
    geom_boxplot(outlier.alpha = 0) +
    geom_point(position = position_jitterdodge(jitter.height = 0, jitter.width = 0.1), alpha = 0.5) +
    scale_color_manual(values = colour.palette, name = colour.by.title) +
    theme_classic() +
    theme(legend.position = legend.position, text = element_text(size=15)) +
    labs(x = x.title, y = y.title) + 
    scale_y_continuous(expand = expansion(mult = c(0, 0.2))) + #add white space to the top of graphs for stats :)
  ...
  if(!is.null(facet)){Plot <- Plot + facet_wrap(facet, scales = facet_scales, nrow = facet_rows)}
  if(y.log == TRUE){Plot <- Plot +  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x), labels=trans_format("log10", math_format(10^.x)), expand = expansion(mult = c(0.05, 0.2)))}
  if(!is.null(x.axis.angle)){Plot <- Plot + theme(legend.position = legend.position, text = element_text(size=15), axis.text.x = element_text(angle = x.axis.angle, hjust = 1))}
...
FACSPlotterXtreme <- function(FACS.data, x.axis, y.axis.as.list, colour.by,
                              ...
                              file.name = "Spleen", file.width = 18.5, file.height = 16, save = FALSE, facet = NULL, facet_scales = "free_x", xblank = FALSE, y.log = FALSE, colour.palette = c("#494949ff", "#7900f5ff"), facet_rows = 1, x.axis.angle = NULL){
  ...
  for (i in 1:length(var_list$cell_plot)){
    ...
    Plot <- FACSPlotter(FACS.data, x.axis, var_list$cell_plot[i], colour.by, colour.by.title = colour.by.title, y.title = var_list$y_label[i], x.title = x.title, legend.position = legend.position, facet = facet, facet_rows = facet_rows, facet_scales = facet_scales, xblank=xblank, y.log = y.log, colour.palette = colour.palette, x.axis.angle=x.axis.angle)
      plot_list[[i]] = Plot
